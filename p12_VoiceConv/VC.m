...
load('C:\Users\Admin\BME\project arshad\result\NN 8 layer-hazfe sokot\NN 8 layer-hazfe sokot.mat'); % load shabakeye talim yafte
subend=199; subfirst=100;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%

while E>0.01
    num_session=5;
    %_____________________________________________________________________
    Q=subfirst:subend; q=randperm(subend-subfirst+1); Q=Q(q); in=Q(1);
    num_subject=in;
    xx=[]; dd=[]; m=0;
    ...
    label=load(['C:\Users\Admin\BME\project arshad\data\LABLES\Zt',num2str(num_session),num2str(100)]);
    label=label.Z;
    ...
    Q=subfirst:subend; q=randperm(subend-subfirst+1); Q=Q(q); out=Q(1);
%     if out == 164 || out== 137 || out==193
%         out=out+1;
%     end
    num_subject=out;
    xx=[]; dd=[]; m=0;
    ...
    for p=1:P
        ...
        b=logsig([b1 1]*u12);
        a=logsig([y1 1]*w1);
        y2=logsig([a 1]*w2+[d_out 1]*u2);% d or b!!??
        z=logsig([y2 1]*v2);
        z2=logsig([z 1]*v3);
        z3=[z2 1]*v4;
        %%
        E=E+sum((zout-z3).^2)+sum((d_in-b).^2);
        dz3=(x0-z3)*AA;%*0.01;
        Z2=[z2 1];
        ...
        dB1=B1.*(1-B1).*(db*u12');
        dB1(n_b1+1)=[];
        du12=eta.*B1'*db+alpha.*du12;
        u12=u12+du12;
        Y1=[y1 1];
        du11=eta.*Y1'*dB1+alpha.*du11;
        u11=u11+du11;

...
%                 end
%                 %%%%%%%%mohasebe energy:%%%%%%%%%%
%                 [s,fs]=wavread ([ 'C:\Users\Admin\BME\project arshad\data\WAV\W',num2str(num_session),num2str(marja),'.WAV']);
%                 energy_marja=sum(s.*s);
%                 energy_smp=sum(smp.*smp);
%                 smp=smp*[sqrt(energy_marja/energy_smp)];
%                 %smp=smp*5;
%                 wavwrite ( smp, fs ,['C:\Users\Admin\BME\project arshad\data\equal ROS waves\W',num2str(num_session),num2str(num_subject),'_to_',num2str(marja),'.wav']);
%                 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                 %makhlot kardane do soot:
%                 S=0;
%                 s=s(1:length(smp));
%                 S=s+smp';
%                 wavwrite ( S', fs ,['C:\Users\Admin\BME\project arshad\data\equal ROS waves\W',num2str(num_session),num2str(num_subject),'_makhloot_',num2str(marja),'.wav']);
 CBw=CB_Z2;       

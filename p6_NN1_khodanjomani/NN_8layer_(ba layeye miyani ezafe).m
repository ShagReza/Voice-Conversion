...
w2=0.1*rand(n_a+1,n_a2);
dw2=0.001*rand(n_a+1,n_a2);
w3=0.1*rand(n_a2+1,n_y2);
dw3=0.001*rand(n_a2+1,n_y2);
...
while E>0.01
    ...
    for p=1:P
        ...
        dA2=A2.*(1-A2).*(dY2*w3');
        dA2(n_a2+1)=[];
        dw3=eta.*A2'*dY2+alpha.*dw3;
        w3=w3+dw3;
        %%
        A=[a 1];
        dA=A.*(1-A).*(dA2*w2');
        dA(n_a+1)=[];
        dw2=eta.*A'*dA2+alpha.*dw2;
        w2=w2+dw2;
        %%
        D=[d 1];
    ...
    save('C:\Users\Admin\BME\project arshad\result\NN_8 ba laye miyani ezafe\NN_8 ba laye miyani ezafe')
    %hold on;
    %plot(e);
    %drawnow;
end
